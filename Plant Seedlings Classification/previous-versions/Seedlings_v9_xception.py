# Using Pic2Vec, take the image features from a pre-trained model (Xception),
# Extract them, allow the top layers to be trainable and save the modified,
# seedlings-specific model weights 

# Original version of Pic2Vec: https://github.com/datarobot/pic2vec
...
from keras.layers import GlobalAvgPool2D, Lambda, average, Dense
...
def build_featurizer(depth_of_featurizer, downsample, num_pooled_features=0,
                     model_str='xception', loaded_model=None):
    """
...
#X = StandardScaler(copy=False).fit_transform(images)
...
for image in images:
    X.append(StandardScaler().fit_transform(image))
...
for i in range(10):
    batch = images[batch_size * i : batch_size * (i+1) -1]
    X_.append(batch)
...
X = images.reshape(-1,image_size,image_size,3)
...
train_generator = datagen.flow(X_train, y=y_train, batch_size=batch_size, seed=42)

# Build pre-trained model

base_model = build_featurizer(depth_of_featurizer=1, downsample=True, num_pooled_features = 8, model_str='xception') 
...
model.fit_generator(train_generator, steps_per_epoch= X_train.shape[0] // batch_size, epochs=epochs, 
                    validation_data=(X_test, y_test), validation_steps=1, verbose=True, callbacks=[early_stopping_monitor])

model.save('xception_2_12_18.h5')
model.save_weights('xception_2_12_18_weights.h5')
