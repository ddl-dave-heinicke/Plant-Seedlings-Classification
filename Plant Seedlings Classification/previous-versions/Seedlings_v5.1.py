# The "version 5" models begin developing a neural net to train on the plant images.
# Versions 5.XX took the pre-trained model weights from the model in this version 
# and modified the final layers to improve performance
...
# 2 Data Augmentation & Sequential Model

# Data Augmentation

vals  = [0.2]

df_zoom = pd.DataFrame(index=vals, columns=['validation_accuracy', 'validation_loss'])
...
for val in vals:
    ...
    for i in range(1):
        ...
        seedlings_train = model.fit_generator(datagen.flow(X_train,y_train, batch_size=batch_size), epochs=epochs, validation_data = (X_test, y_test), steps_per_epoch= X.shape[0] // 86, callbacks=[early_stopping_monitor], verbose=True)

        val_accuracies.append(np.mean(seedlings_train.history['val_acc']))
        val_losses.append(np.mean(seedlings_train.history['val_loss']))
        
    
    df_zoom.validation_accuracy[val] = np.mean(val_accuracies)
    df_zoom.validation_loss[val] = np.mean(val_losses)

print(df_zoom)

model.save('CNN_3block_1_22_18.h5')
model.save_weights('CNN_3block_1_22_18_weights.h5')
...
#df_zoom.to_csv('df_zoom_rot.csv', index=True)
