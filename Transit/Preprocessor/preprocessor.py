...
for k in df_dict.keys():
    ...
    dollar_cols = ['Fares_FY', 'Operating_Expenses_FY',
                   'Average_Cost_per_Trip_FY', 'Average_Fares_per_Trip_FY']

    if k == 'Master':
        for col in dollar_cols:
            df_dict['Master'][col].fillna('0', inplace=True)
            df_dict['Master'][col] = df_dict['Master'][col].apply(
                lambda x: x.replace('-', '0'))
            df_dict['Master'][col] = df_dict['Master'][col].apply(
                lambda x: x.lstrip().lstrip('$').rstrip().replace(',', ''))
