...
def resample_and_balance(X, y, num=1000):
    ...
    df_fraud_resample = df_fraud_resample.sample(n=num - num_original,
                                                 replace=False)
...
print('There are %i instances of fraud from %i transactions' % (np.sum(y),
                                                                len(y)))

# Set aside 25% of data to test on later

X, X_holdout, y, y_holdout = train_test_split(X, y, test_size=0.25,
                                              random_state=42)
...
ax = sns.regplot(fraud[:, 0], fraud[:, 1], color='r', scatter=True,
                 fit_reg=False, scatter_kws={'s': 4})
ax = sns.regplot(legit[:, 0], legit[:, 1], color='b', scatter=True,
                 fit_reg=False, scatter_kws={'s': 4})
...
logreg_coefs_o = StandardScaler().fit_transform(logreg_coefs_o[0]
                                                .reshape(-1, 1))
...
logreg_coefs_r = StandardScaler().fit_transform(logreg_coefs_r[0]
                                                .reshape(-1, 1))
...
ax.legend(['Logreg Feature Weights', 'XGBoost Feature Weights',
           'Light GB Feature Weights'], loc=(0.6, 0.08), fontsize='large')
